...
def add_user():
  if request.method == 'POST':
    ...
    if (username == '' or birthday == null or email == ''):
      ...
      cmd2 = "insert into Users (user_id, username, DOB, email, isGenUser, isSuperUser) values (:w, :n, :y, :z, TRUE, FALSE)"
      cursor = g.conn.execute(text(cmd2), w = pkeystr, n = username, y = birthday, z = email)
      cursor.close()
